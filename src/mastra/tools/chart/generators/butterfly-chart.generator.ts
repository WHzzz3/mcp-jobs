...
export interface ButterflyChartInput extends BaseChartInput {
  data: Array<Array<(string | number)[]>>; // [分类, 左值, 右值]
  title?: string;
  subtitle?: string;
  showLabels?: boolean;
...
export class ButterflyChartGenerator extends BaseChartTool {
  ...
  async generateConfig(input: ButterflyChartInput): Promise<ButterflyChartOutput> {
    // 验证输入
    ...
    const colors = validatedInput.colors || ['#FF6B6B', '#4ECDC4']; // 左右两侧默认颜色
