...
// 基础雷达图特定输入接口
export interface BasicRadarChartInput extends Omit<BaseChartInput, 'chartType'> {
  data: Array<Array<string | number>>; // [维度, 系列1, 系列2, ...] 格式
...
export class BasicRadarChartGenerator extends BaseChartTool {
  ...
  getChartMetadata() {
    return {
      ...
      optionalFields: ['标题', '副标题', '线条类型', '线条宽度', '填充透明度', '标签显示']
    };
  }
} 