...
export const MixedLineGroupedColumnChartInputSchema = BaseChartInputSchema.extend({
...
export class MixedLineGroupedColumnChartGenerator extends BaseChartTool {
  ...
  async generateConfig(input: MixedLineGroupedColumnChartInput): Promise<MixedLineGroupedColumnChartOutput> {
    // 验证输入
    const validatedInput = MixedLineGroupedColumnChartInputSchema.parse(input);
    const inputWithChartType = { ...validatedInput, chartType: 'mixed-line-grouped-column' };
    ...
    if (dataCols < 3) {
      throw new Error('混合线条-分组柱状图需要至少3列数据（X轴 + 至少2个数值列）');
    }
    ...
    const defaultColumnIndices = Array.from({ length: totalValueCols - 1 }, (_, i) => i + 2);
  ...
  async loadSchema(): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to load mixed-line-grouped-column schema: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
} 