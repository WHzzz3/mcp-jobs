...
export interface MixedLineStackedColumnChartInput extends BaseChartInput {
  data: Array<Array<string | number>>; // 多列数据，第一列为X轴，后续列为数值
  colors?: string[];
...
export const MixedLineStackedColumnChartInputSchema = BaseChartInputSchema.extend({
  ...
  leftYAxisConfig: z.object({
    title: z.string().optional(),
    unit: z.string().optional(),
  }).optional(),
  rightYAxisConfig: z.object({
    title: z.string().optional(),
    unit: z.string().optional(),
  }).optional(),
});
...
export class MixedLineStackedColumnChartGenerator extends BaseChartTool {
  ...
  async generateConfig(input: MixedLineStackedColumnChartInput): Promise<MixedLineStackedColumnChartOutput> {
    // 验证输入
    const validatedInput = MixedLineStackedColumnChartInputSchema.parse(input);
    const inputWithChartType = { ...validatedInput, chartType: 'mixed-line-stacked-column' };
    ...
    if (dataCols < 4) {
      throw new Error('混合线条-堆叠柱状图需要至少4列数据（X轴 + 至少3个数值列）');
    }

    // 确定默认的列索引分配
    const totalValueCols = dataCols - 1; // 减去X轴列
    const defaultColumnCount = Math.ceil(totalValueCols * 0.6); // 60%作为柱状图
    const defaultColumnIndices = Array.from({ length: defaultColumnCount }, (_, i) => i + 1);
    const defaultLineIndices = Array.from({ length: totalValueCols - defaultColumnCount }, (_, i) => i + 1 + defaultColumnCount);
    ...
    // 获取默认配置
    const themeColors = getThemeColors(mergedInput.theme || 'light', totalSeries);
    ...
    const yAxis = [
      // 左Y轴（柱状图）
      {
        ...
        position: "left",
        title: {
          show: !!validatedInput.leftYAxisConfig?.title,
          text: validatedInput.leftYAxisConfig?.title || "",
          fontFamily: "Misans 常规",
          fontSize: 12,
          color: { color: "#333333", opacity: 1 }
        },
      ...
      // 右Y轴（线条图）
      {
        ...
        position: "right",
        title: {
          show: !!validatedInput.rightYAxisConfig?.title,
          text: validatedInput.rightYAxisConfig?.title || "",
          fontFamily: "Misans 常规",
          fontSize: 12,
          color: { color: "#333333", opacity: 1 }
        },
  ...
  async loadSchema(): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to load mixed-line-stacked-column schema: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
} 