...
export class StackedColumnChartGenerator extends BaseChartTool {
  ...
  async generateConfig(input: StackedColumnChartInput): Promise<StackedColumnChartOutput> {
    // 验证输入
    const validatedInput = StackedColumnChartInputSchema.parse(input);
    const inputWithChartType = { ...validatedInput, chartType: 'stacked-column' };
  ...
  async loadSchema(): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to load stacked-column schema: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
} 